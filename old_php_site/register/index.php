...
<body>
	...
	<div id="content">
		...
		<p>
			...
			BarCamp Rochester, Spring 2013 registration hasn't opened yet. If you want to get a better idea of what happens at BarCamp, 
			check out our <a href="../howto">How-To: BarCamp Rochester</a> page. Follow us on twitter <a href="http://twitter.com/barcamproc" target="_new">@BarCampRoc</a>, and you can get the latest updates on the event.	
		</p>
		<!--
		...
		<form id="regForm" action="proc_reg.php" method="POST">
			...
			?>
			<label id="l" for="email">E-Mail:</label><input id="email" name="email"  type="text" value="<?php if(isset($_REQUEST['email'])) { echo($_REQUEST['email']); } ?>" />			
			...
			<label for="fname">First Name:</label><input id="fname" name="fname" type="text" value="<?php if(isset($_REQUEST['fname'])) { echo($_REQUEST['fname']); } ?>" />
			<p>Your first name will show up in the registered attendees list. </p>
			<label for="lname">Last Name:</label><input id="lname" name="lname" type="text" value="<?php if(isset($_REQUEST['lname'])) { echo($_REQUEST['lname']); } ?>" />
			...
			though!</p>
			<label for="topic">Topic:</label><input id="topic" name="topic" type="text" value="<?php if(isset($_REQUEST['topic'])) { echo($_REQUEST['topic']); } ?>" />
			...
			<div id="specialShirtNote">Our t-shirt will be placed Friday, September 21st. We can't be sure of any extras! Register before then to guarantee you'll get a shirt.</div>
			...
			<?php if(isset($_REQUEST['shirt'])) { 
				echo('<select id="shirt" name="shirt">');
				if ($_REQUEST['shirt'] == 'S') {
				...
				if ($_REQUEST['shirt'] == 'XL') {
					echo('<option selected="selected">XL</option>');
				} else {
					echo('<option>XL</option>');
				}
				if ($_REQUEST['shirt'] == 'XXL') {
					echo('<option selected="selected">XXL</option>');
				} else {
			...
			<?php } ?>
			[<a id="sizingChart" href="#">sizing chart</a>]
			<p>So you can fit in to your free shirt, rather than walk around naked.</p>
			<label for="twitter">Twitter:</label><input id="twitter" name="twitter" type="text" value="<?php if(isset($_REQUEST['twitter'])) { echo($_REQUEST['twitter']); } ?>" />
